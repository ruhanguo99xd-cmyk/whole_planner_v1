...
def do_one_dig_cycle(plc, perception_node: Optional[PerceptionCommNode] = None):
    ...
    wait_until(
        name="卸载动作执行并完成",
        node=perception_node,
        on_enter=lambda: xieliao_function(plc),
        is_done=lambda: plc_done(plc),
        timeout_sec=60.0,
        poll_sec=0.05,
    )
    ...
    wait_until(
        name="卸载复位动作执行并完成",
        node=perception_node,
        on_enter=lambda: fuwei_function(plc),
        is_done=lambda: plc_done(plc),
        timeout_sec=60.0,
        poll_sec=0.05,
    )
...
def main():
    ...
    try:
        ...
        # ===================== 第一次：回转90° + 感知 =====================
        # ！！！！！每次运动前开斗都需要清0，避免上一次程序中断未清0导致第一斗出现关不上斗的问题
        status_byte = plc.db_read(1301, 12040, 1)
        util.set_bool(status_byte, 0, 0, 0)
        plc.db_write(1301, 12040, status_byte)
        print("关闭开斗状态")
        ...
        while rclpy.ok():
            print("\n" + "="*50)
            print(f"开始第 {loop_idx} 轮：挖掘-卸载-开斗-感知(物料)-复位")
            print("="*50)

            # 一轮完整连续机械动作：挖掘→卸载→开斗→感知(物料)→复位
